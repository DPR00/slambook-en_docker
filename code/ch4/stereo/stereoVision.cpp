...
int main() {
    ...
    cv::Ptr<cv::StereoSGBM> sgbm =
        cv::StereoSGBM::create(0, 96, 9, 8 * 9 * 9, 32 * 9 * 9, 1, 63, 10, 100, 32);  // SGBM is sensitive to parameters
    ...
    for (int v = 0; v < left.rows; v++) {
        for (int u = 0; u < left.cols; u++) {
            ...
            Eigen::Vector4d point(
                0, 0, 0, left.at<uchar>(v, u) / 255.0);  // the first three dimensions are xyz, the 4-th is the color

            // compute the depth from diparity
            double x = (u - cx) / fx;
            double y = (v - cy) / fy;
